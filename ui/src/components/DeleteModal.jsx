import { useGetCommentsQuery, useDeleteCommentMutation, useDeleteReplyMutation} from "../slices/commentSlice.js";
/* eslint-disable react/prop-types */
/* eslint-disable react/no-unescaped-entities */
const DeleteModal = (props) => {

    const {commentId, setOpenDeleteModal, parentComment, isReply} = props;
    ...
    const handleDelete = async () => {
      if(isReply){
        try {
          await deleteReply({replyId:commentId, _id:parentComment._id});
          refetch()
  ...
  return (
    <div className="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-black/50">
        ...
        <div className="mb-5 flex justify-between text-white font-bold">
            ...
            <button onClick={handleDelete} className="p-3 bg-red-600 rounded">YES, DELETE</button>
        </div>
    </div>
    </div>
  )
